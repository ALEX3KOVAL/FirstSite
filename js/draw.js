$(function(){
  var cx = document.getElementById("draw__canvas_left_line").getContext("2d");
  cx.strokeStyle = "blue";
  cx.lineJoin = "round";
  cx.lineWidth = 2;
  cx.beginPath();
  cx.moveTo(0, 10);
  cx.lineTo(605, 10);
  cx.stroke();
  var cx_rect = document.getElementById("draw__canvas_rect").getContext("2d");
  cx_rect.strokeStyle = "blue";
  cx_rect.lineJoin = "round";
  cx_rect.lineWidth = 2;
  cx_rect.strokeRect(1,2,295,125);
  cx_rect.shadowColor = "blue";
  cx_rect.shadowOffsetX = 0;
  cx_rect.shadowOffsetY = 0;
  cx_rect.shadowBlur = 10;
  cx_rect.strokeStyle = "#F00";
  cx_rect.font = "Bold 60pt Helvetica Neue,Helvetica, Arial, sans-serif";
  cx_rect.fillStyle = "#ff8cff";
  cx_rect.fillText("О б о  м н е", 55, 90, 190);
  var cx_right = document.getElementById("draw__canvas_right_line").getContext("2d");
  cx_right.strokeStyle = "blue";
  cx_right.lineJoin = "round";
  cx_right.lineWidth = 2;
  cx_right.beginPath();
  cx_right.moveTo(0, 10);
  cx_right.lineTo(300, 10);
  cx_right.stroke();
})